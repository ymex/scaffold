...
func TestKson_Encode(t *testing.T) {
	tim :=time.Now().Unix()
	fmt.Println(tim)
	fmt.Println(time.Unix(tim,0).Format("2006-01-02"))
}

func TestKson_Find(t *testing.T) {
	var email = ""
	if !validate.IsEmail(email) && len(email) != 0{
		fmt.Println("邮箱不合法。")
		return
	}
}
