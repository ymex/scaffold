...
//加密
func GenerateScryptHash(text string) (string, error)  {
	hash,err := GenerateFromPassword([]byte(text),DefaultParams);
	if err != nil{
		return "",err
	}
...
//匹配
func CompareScryptHash(hash string, text string) error {
	return CompareHashAndPassword([]byte(hash),[]byte(text))
}
